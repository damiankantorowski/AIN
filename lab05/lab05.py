...
def evaluate1(x):
    ...
    result = -expr1 + np.sin(cotangent)
    if not evaluations or result < evaluations[-1]:
...
def save_ecdf(data, n, evaluate, fout):
    ...
    np.savetxt(fout, np.array(ecdf).T, fmt="%d", delimiter=",")
...
def main():
    ...
    for func in [1, 2]:
        for repre in ["real", "binary"]:
            print(f"Function {func}, Representation {repre}")
            evaluate = evaluate1 if func == 1 else evaluate2
            ...
            for i in range(100):
                ...
                experiments.append(evaluations)
            data = np.array(experiments).T
            np.savetxt(f"{repre}_{func}.csv", data, fmt='%.8f', delimiter=",")
            save_ecdf(data, args.n, evaluate, f"ecdf_{repre}_{func}.csv")
