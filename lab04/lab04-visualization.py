...
def main():
    ...
    fig, ax = plt.subplots()  # Properly define ax here
    ax.set_xlabel('Generations')
    ax.set_ylabel('Objective Value')

    gen_min_scores, gen_max_scores, gen_mean_scores, gen_best_so_far = [], [], [], []
            
    min_line, = ax.plot([], [], label="Min", color="blue")
    max_line, = ax.plot([], [], label="Max", color="red")
    mean_line, = ax.plot([], [], label="Mean", color="green")
    best_line, = ax.plot([], [], label="Best So Far", color="purple")
    ax.legend()
    ...
    for i in range(1, 11):
        ...
        with open(filename, "w") as file:
            ...
            while evaluate.cost < MAX_COST:
                ...
                gen_min_scores.append(np.min(scores))
                gen_max_scores.append(np.max(scores))
                gen_mean_scores.append(np.mean(scores))
                if (new_min := np.min(scores)) < best_so_far:
                    best_so_far = new_min
                gen_best_so_far.append(best_so_far)

                # Update the plot data
                min_line.set_data(range(len(gen_min_scores)), gen_min_scores)
                max_line.set_data(range(len(gen_max_scores)), gen_max_scores)
                mean_line.set_data(range(len(gen_mean_scores)), gen_mean_scores)
                best_line.set_data(range(len(gen_best_so_far)), gen_best_so_far)
                ax.relim()  # Rescale axes
                ax.autoscale_view()
